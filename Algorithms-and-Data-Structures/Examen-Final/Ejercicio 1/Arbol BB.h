...
private:
	...
	bool _buscarPorMarca(Nodo<T>* nodo, const string& marca, T& resultado) {
		if (nodo == nullptr) return false;

		// Comparar la marca con el nodo actual
		if (nodo->elemento.marca == marca) {
			resultado = nodo->elemento;
			return true;
		}

		// Buscar en los subárboles
		if (_buscarPorMarca(nodo->izq, marca, resultado)) return true;
		return _buscarPorMarca(nodo->der, marca, resultado);
	}
public:
	...
	bool buscarPorMarca(const string& marca, T& resultado) {
		return _buscarPorMarca(raiz, marca, resultado);
	}
};